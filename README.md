...
func (s *Searcher) Debug(
        idx *index.Indexer,   // 索引
        q query.Query,        // query
        e operation.Operation,  // 自定义的operation对象
        ids []document.DocId    // 被过滤掉的id
        ) map[document.DocId]map[string]interface{}
         // 返回每一个id对应的过滤信息，过滤信息字段和query序列化后一致，在每一个字段里添加了过滤的原因
// 返回的格式内容
{
    "10":{
        "and":[
            {"=":["fieldName","1","id found"]},
            {"=":["fieldNeme","2","id not found"]},
            {"=":["fieldName","3","id not found"]},
            {
                "and":[
                    {"=":["fieldName","1","id found"]},
                    {"=":["fieldNeme","2","id not found"]},
                    {"=":["fieldName","3","id not found"]},
                    {
                        "or":[
                            ...
                            {"=":["fieldNeme","2","id not found"]},
                            {"=":["fieldName","3","id not found"]},
                            {"=":["fieldName","4","id not found"]}
                        ],
                        "or_check":[
                            {"check":["fieldName",2,0,0,false,"=","check result false"]},
                            {"and_check":[
                                {"check":["fieldName",2,0,0,false,"=","check result false"]},
                                {"check":["fieldName",3,0,0,false,"=","check result true"]}
                            ]}
                        ]
                    }
                ],
                "and_check":[
                    {"in_check":["fieldName",[2,3,4,5],0,false,"check result true"]},
                    {
                        "or_check":[
                            {"check":["fieldName",2,1,0,false,"!=","check result true"]},
                            {"check":["fieldName",3,0,0,false,"=","check result true"]}
                        ]
                    }
                ]
        ...
        "and_check":[
            {"in_check":["fieldName",[2,3,4],0,false,"check result true"]},
            {
                "or_check":[
                    {"check":["fieldName",2,5,0,false,"\u003e","check result true"]},
                    {"check":["fieldName",3,0,0,false,"=","check result true"]}
                ]
            }
        ]
